...
import com.itsuda.sourceBoard.service.Reply.SoBoardReplyService;
...
@Log4j
@RestController
@RequestMapping("/SoReplies")
public class SoBoardReplyController extends UriMap{
	
	private final SoBoardReplyService replyService;
	
	@Inject
	public SoBoardReplyController(SoBoardReplyService replyService) {
...
//	댓글 등록처리 
	...
	public ResponseEntity<String> register(@RequestBody SoBoardReplyVO replyVO){
		log.info("댓글 등록처리");
...
//	대댓글 등록처리 
	...
	public ResponseEntity<String> comments(String pReplyNo, String pSeq, String cText, String cWriter){
		log.info("대댓글 등록처리");
		ResponseEntity<String> entity = null;
		SoBoardReplyVO vo = new SoBoardReplyVO();
...
//	댓글 목록 
	...
	public ResponseEntity<List<SoBoardReplyVO>> list(@PathVariable("seq") Integer seq){
		log.info("댓글 목록");
		ResponseEntity<List<SoBoardReplyVO>> entity = null;
...
//	댓글 수정 처리
	...
	public ResponseEntity<String> update(@PathVariable("replyNo") Integer replyNo, @RequestBody SoBoardReplyVO replyVO){
		log.info("댓글 수정 처리");
		ResponseEntity<String> entity = null;
		try {
...
//	댓글 목록 출력 및 페이징 처리 
	...
	public ResponseEntity<Map<String, Object>> listPaging(@PathVariable("seq") Integer seq
		...
		try {
			...
			List<SoBoardReplyVO> replies = replyService.getRepliesPaging(seq, searchCriteria);
