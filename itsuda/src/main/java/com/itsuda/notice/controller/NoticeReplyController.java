...
import com.itsuda.notice.service.Reply.NoticeReplyService;
...
public class NoticeReplyController extends UriMap{
	
	private final NoticeReplyService replyService;
	
	@Inject
	public NoticeReplyController(NoticeReplyService replyService) {
...
//	댓글 등록처리 
	...
	public ResponseEntity<String> register(@RequestBody NoticeReplyVO replyVO){
		log.info("댓글 등록처리");
...
//	댓글 목록 
	...
	public ResponseEntity<List<NoticeReplyVO>> list(@PathVariable("seq") Integer seq){
		log.info("댓글 목록");
...
//	댓글 수정 처리
	...
	public ResponseEntity<String> update(@PathVariable("replyNo") Integer replyNo, @RequestBody NoticeReplyVO replyVO){
		log.info("댓글 수정 처리");
		ResponseEntity<String> entity = null;
		try {
...
//	댓글 목록 출력 및 페이징 처리 
	...
	public ResponseEntity<Map<String, Object>> listPaging(@PathVariable("seq") Integer seq
		...
		try {
			...
			List<NoticeReplyVO> replies = replyService.getRepliesPaging(seq, searchCriteria);
