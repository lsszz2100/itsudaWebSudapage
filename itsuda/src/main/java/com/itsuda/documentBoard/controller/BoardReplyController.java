...
import com.itsuda.documentBoard.service.Reply.BoardReplyService;
...
@Log4j
@RestController
@RequestMapping("/DoReplies")
public class BoardReplyController extends UriMap{
...
//	댓글 목록 
	...
	public ResponseEntity<List<BoardReplyVO>> list(@PathVariable("seq") Integer seq){
		log.info("댓글 목록");
...
//	댓글 수정 처리
	...
	public ResponseEntity<String> update(@PathVariable("replyNo") Integer replyNo, @RequestBody BoardReplyVO replyVO){
		log.info("댓글 수정 처리");
		ResponseEntity<String> entity = null;
		try {
...
//	댓글 목록 출력 및 페이징 처리 
	...
	public ResponseEntity<Map<String, Object>> listPaging(@PathVariable("seq") Integer seq
		...
		try {
			...
			List<BoardReplyVO> replies = replyService.getRepliesPaging(seq, searchCriteria);
